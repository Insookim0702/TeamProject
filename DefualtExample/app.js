...
var bot = require('624691290:AAHOxz1XBTvt3BSwEV1SXHgbsN_E7949Zx8', {pollong:true});*/
...
/*===============게시판 보여주기==============*/
var p1 = require('./board/boardlist');
app.use('/process', p1);

/*============게시판 글읽기& hit 수 증가시키기==*/
var boardread = require('./board/boardread');
app.use('/process', boardread);

/*===============게시판 보여주기==============*/
app.get('/process/boardlist/:no', (req,res) =>{
    var no = req.params.no;
     console.log('/process/boardlist 라우터 함수를 호출');
     pool.getConnection(function(err, conn){
        if(err){
             console.log('DB접속오류발생');
            if(conn){
                conn.release();
            }
            callback(err, null);
    ...
    fs.readFile('board.ejs','utf-8',(error, data) =>{
        ...
        conn.query('select no, title, writer, hit, DATE_FORMAT(moddate, "%Y/%m/%d %T") as moddate from board1 order by no desc', function(err, rows){
            ...
            /*let list =[];
            var listcount = parseInt(rows.length/10);
            var cou = parseInt(no*10);
            console.log(cou);
            var cou =cou-10;
            console.log(cou);
            //for(let i=0; i<=listcount;i++){
                for(let j=cou;j<rows.length;j++){
                    list.push(rows[j]);
                }*/
            //}
            console.log(no);
            console.log(list);
            //console.log(list[no]);
            res.render('board', {rows: list }); // view 디렉토리에 있
        });
    });
     });
});
...
app.get('/delete/:id', function(req, res){
    ...
    console.log('데이터베이스 연결 스레드 아이디 :' + conn.threadId);
    /*게시글 개수 -1*/
    conn.query('UPDATE boardcount SET count = count-1',function(){});
...
app.post('/board/write',function(req,res,next){
    ...
    pool.getConnection(function(err, conn){
        ...
        conn.beginTransaction(function(err){
            /*게시글 개수 -1*/
    conn.query('UPDATE boardcount SET count = count+1',function(){});
            console.log('boardcount에 count+1');
...
router.route('/process/adduser').post(function(req, res){
    ...
    var age = req.body.age || req.query.age;
    var gender = req.body.gender || req.query.gender;
    console.log('요청 파라미터 : '+ email+', '+ name +', '+ age +', ' +gender);
    
    naverinfo(email, name, age, gender,  function(err, addedNaveruser){
        if (err) { //동일한 아이디로 추가할 때 오류 발생 - 클라이언트 오류 전송
            console.log('에러발생.');
                res.writeHead(200, {"Content-Type":"text/html;charset=utf8"});
                res.write('<h1>에러 발생 </h1>');
        ...
        //결과 객체 있으면 성공 응답 전송
        if(addedNaveruser) {
            console.dir(addedNaveruser);
            res.redirect('/naverloginafter.html');
            /*res.writeHead(200,{"Content-Type":"text/html;charset=utf8"});
            res.write('<h1>사용자 추가 성공</h1>');*/
            res.end();
           }//addedUser가 없으면
...
/*===============네이버 회원 정보 가져오기================*/
var naverinfo = function(email, name, age, gender, callback){
    alert(111);
    console.log('addUser 호출됨.');
